...
if (!preg_match($regex_name, $_POST['name']) || !preg_match($regex_email, $_POST['email']) || !preg_match($regex_password, $_POST['password'])) {
	...
	if (count($arr_login) != 0) {
		...
		// $sql = "INSERT INTO users(name,email,password,timestamp,user_type,food_type) VALUES(?,?,?,?,?,?)";
		
		if($user=="resturant"){
			$sql = "INSERT INTO resturants(name,email,password,timestamp,user_type,food_type) VALUES(?,?,?,?,?,?)";
		}else{
			$sql = "INSERT INTO users(name,email,password,timestamp,user_type,food_type) VALUES(?,?,?,?,?,?)";
		}

		$query  = $pdoconn->prepare($sql);
	    if ($query->execute([$name, $email, $password, $timest, $user, $food])) {
